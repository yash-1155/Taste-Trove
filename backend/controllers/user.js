...
const passwordResetRedirect=async(req,res)=>{
    ...
    const token1=crypto.createHash('sha256').update(idToken.token).digest('hex')
    ...
    res.redirect(`http://localhost:5173/login/resetPassword/${idToken.token}`)
}
module.exports={signUpform,login,signUp,logout,forgotPassword,resetPassword,passwordResetRedirect}
