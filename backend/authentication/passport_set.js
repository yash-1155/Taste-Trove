...
passport.deserializeUser(async (id, done) => {
  try {
    const user1 =
      (await adminUser.findById(id)) || (await gooUser.findById(id));
    if (user1) {
...
passport.use(
  ...
  new GoogleStrategy(
    {
      callbackURL:
        "https://taste-trove.onrender.com/admin/auth/google/redirect",
    ...
    async (accessToken, refreshToken, profile, done) => {
      try {
        ...
        console.log("Admin is: ", profile.emails[0].value);
        if (currentUser) {
...
passport.use(
  ...
  new GoogleStrategy(
    {
      callbackURL: "https://taste-trove.onrender.com/auth/google/redirect",
      clientID: process.env.clientID,
      clientSecret: process.env.clientSecret,
    },
    async (req, accessToken, refreshToken, profile, done) => {
      try {
        ...
        console.log("User is: ", profile.emails[0].value);
        if (currentUser) {
